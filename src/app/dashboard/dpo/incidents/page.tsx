...
type Incident = { id: string; title: string; severity: string; status: string; description: string; containment_notes?: string; occurred_at: string; created_at: string }
export default function IncidentsPage() {
  ...
  const changeStatus = async (id: string, status: string) => { const { error } = await supabase.from('dpo_incidents').update({ status }).eq('id', id); if (error) setError(error.message); else { await logActivity('dpo.incident.status', 'dpo_incident', id, { status }); load() } }
  return <div><PageHeader title="Incident Log" description="Append-only breach and privacy incident record." />{error && <p className="mb-4 rounded-lg bg-rust/10 p-3 text-sm text-rust">{error}</p>}
    ...
    <div className="space-y-3">{items.map(item => <article key={item.id} className="card"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="font-semibold text-ink">{item.title}</h2><div className="flex items-center gap-2"><span className="rounded-full bg-rust/10 px-3 py-1 text-xs font-semibold uppercase text-rust">{item.severity}</span><select value={item.status || 'open'} onChange={e => changeStatus(item.id, e.target.value)} className="px-2 py-1 text-xs capitalize"><option value="open">Open</option><option value="contained">Contained</option><option value="resolved">Resolved</option></select></div></div><p className="mt-2 text-sm text-muted">Occurred {new Date(item.occurred_at).toLocaleString('en-UG')}</p><p className="mt-3 text-sm">{item.description}</p>{item.containment_notes && <p className="mt-2 text-sm"><strong>Containment:</strong> {item.containment_notes}</p>}</article>)}</div>
  </div>
}
