...
export default function ProcessingRegisterPage() {
  ...
  const remove = async (id: string) => { if (!confirm('Remove this processing activity?')) return; const { error } = await supabase.from('processing_activities').delete().eq('id', id); if (error) setError(error.message); else load() }
