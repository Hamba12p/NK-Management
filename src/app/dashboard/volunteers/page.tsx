    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900">{volunteers.filter(v => v.volunteer_status === 'active').length}</p>
            </div>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Volunteer Leads</p>
              <p className="text-3xl font-bold text-slate-900">{volunteers.filter(v => v.volunteer_tier === 'volunteer_lead').length}</p>
            </div>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900">{volunteers.reduce((sum, v) => sum + (v.volunteer_hours || 0), 0)}</p>
            </div>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Avg Hours/Volunteer</p>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-slate-900">Volunteer Directory</h2>
        {volunteers.length === 0 ? (
          <div className="bg-white rounded-lg border p-12 text-center">
            <Users className="w-12 h-12 text-slate-300 mx-auto mb-4" />
            <p className="text-slate-600">No volunteers yet</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {volunteers.map((vol) => (
              <div key={vol.id} className="bg-white rounded-lg border p-6 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-pink-100 to-purple-100 flex items-center justify-center font-semibold text-pink-700">
                      {vol.full_name?.[0] ?? '?'}
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-slate-900">{vol.full_name}</p>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-gray-500">{vol.volunteer_department}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-slate-900">{vol.volunteer_hours || 0}</p>
